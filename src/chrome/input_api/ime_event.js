...
function GetImeKeyCode(k) {
  return JavaScriptKeyCodeTable[k] || "";
}

export function CreateImeKeyEvent(ev) {
  ...
  let code = GetImeKeyCode(ev.keyCode);
  let key = jscin.get_key_val(code);

  if (key.length == 1) {
    if (ev.shiftKey) {
      let shift_map = '~!@#$%^&*()_+{}:"<>?|';
      let char_map =  "`1234567890-=[];',./\\";
      let index = char_map.indexOf(key);
      if (index >= 0)
        key = shift_map[index];
    } else {
      if (key >= 'A' && key <= 'Z')
